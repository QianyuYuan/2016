...
```{r}
roullete<-rep(c("black","red","green"),times=c(18,18,2))
get_outcome<-function(n){
  earning<-sample( ifelse( roullete=="black", 1,-1),  n, replace = TRUE)
  sum(earning)
}
...
n<-c(10,25,100,1000)
winning<-function(n){
  rep<-replicate(10000, get_outcome(n))
  mean<-mean(rep,na.rm=T)
  SD<-sd(rep,na.rm=T)
  SE<-SD/sqrt(length(rep))
  c(mean,SD,SE,hist(rep))
} 
s_n<-sapply(n,winning)
s_n

average<-function(n){
  rep<-replicate(10000, get_outcome(n)/n)
  mean<-mean(rep,na.rm=T)
  SD<-sd(rep)
  SE<-SD/sqrt(length(rep))
  c(mean,SD,SE,hist(rep))
} 
s_nn<-sapply(n,average)
s_nn

```

Your answer here.
1.With 10000 replications, earnings of n plays are normally distributed
2. The absoulte expected values and standard errors incresase as N increase,indicating that expected loss on the roullete increase when we play more times.
3.The expected values and standard errors of average winnings keep almost constant. Expected values are smaller than 0, indicating that we are more like to loss money rather than earning money.
...
The expected value of earnning money whit 10 times play is -0.5914 with standard deviation of 3.1166
...
```{r}
play25<-replicate(10000, get_outcome(25))
tab <- table(play25>0)
prop.table(tab)
```
...
```{r}
casinoloss<-function(n){
  rep<-replicate(10000, get_outcome(n))
  x<-as.numeric(rep>0)
  mean(x)
} 

casinolossp<-sapply(n,casinoloss)
library("ggplot2")
qplot(x=n,y=casinolossp,geom="point")

```

Your answer here. 
The probabilities that casino loses money decrease with the incresase of play times.
...
```{r}
default <- sample( c(-120000,0), 1000, replace = TRUE, prob=c(0.02, 0.98))
lose <-sum(default)
```
...
Your solution here. 
```{r}
rate<-(100+120000*0.02)/(180000*0.98)

interest_rep<-replicate(10000,{
 interest<-sample( c(-120000,180000*rate), 1000, replace = TRUE,
                   prob=c(0.02, 0.98))
 sum(interest)
})
mean(interest_rep)
...
p<-0.98
Z<-qnorm(0.9995)
rate2<-(Z+(1-p)*sqrt(p*(1-p)))*120000/((p*sqrt(p*(1-p))-Z)*180000)

interest2<-function(n){
  foreclosure<-sample(default,n,replace=TRUE)
  x<-as.numeric(foreclosure=="no")
  y<-as.numeric(foreclosure=="yes")
  z=180000*0.02*sum(x)-120000*sum(y)
  return(z)
}
interest_rep2<-replicate(10000,interest2(1000))
tab2<-table(interest_rep2<0)
prop.table(tab2)
```
